...
<div class="w-10/12 md:w-3/5 lg:w-1/3 mx-auto space-y-4 pb-4">
  <ul class="space-y-4">
    {#each users as user}
      <li>
        <div class="px-4 py-2 bg-zinc-100 rounded-md shadow-md flex flex-col space-y-2">
          ...
          <div>
            {#if $session.user.rank > user.rank}
              <button class="hover:text-red-600 hover:bg-zinc-200 rounded-md px-2 py-1 cursor-pointer">
                <Ban /> 정지
              </button>
              <button class="hover:text-red-600 hover:bg-zinc-200 rounded-md px-2 py-1 cursor-pointer">
                <PermBan /> 영구정지
              </button>
              {#if $session.user.rank >= EUserRanks.Admin}
                <button class="hover:text-emerald-500 hover:bg-zinc-200 rounded-md px-2 py-1 cursor-pointer">
                  <SetRole /> 역할 설정
                </button>
              {/if}
              {#if $session.user.rank >= EUserRanks.Admin}
                <button class="hover:text-amber-500 hover:bg-zinc-200 rounded-md px-2 py-1 cursor-pointer">
                  <Promote /> 관리자로 지정
                </button>
              {/if}
            {/if}
